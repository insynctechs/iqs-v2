...
<asp:Content ID="Content1" ContentPlaceHolderID="MainContent" runat="server">   
    ...
    <script defer src="/Scripts/company_profile.js"></script>
    <script defer src="/Scripts/fb.js"></script>    
   <asp:PlaceHolder runat="server">
    ...
    <input type='hidden' id='hdnEmailCaptcha' value="no" />
     <div id="copro-main" class="section" style="padding-top:50px;">  		 
        ...
        <% if (ShowReviews == "Y" && (Int32.Parse(CompRating) > 0))
            { %>
        <div class="col s12 m12 l4 xl4"><div class='divratingnew'><span  id='spanRateHead'>OVERALL RATING: </span>
            ...
            <script type='text/javascript'>
                $(document).ready(function () {
                    ...
                    $('input[type=radio].topcommentstar').rating('select', -1, false);
        ...
        <div class='row'>
            ...
            <div id="divAddress">
                ...
                <a href="<%:RootPath %>copro-map.html?address=<%=MapAddress %>&comp=<%=ClientNameFormatted %>" id="lnkViewMap" class="iframe coproviewmap">View Map</a></span>
                    ...
                    <script type="text/javascript">
                        $(document).ready(function () {
                            $('#lnkViewMap').fancybox({
                                'padding': 0,'showCloseButton': true, 'modal': true, 'titleShow': false, 'autoScale': false,
                                'width': 406, 'height': 366, 'transitionIn': 'none', 'transitionOut': 'none'
                            });
                        });
                </script>
            </div>
                ...
                <script type="text/javascript">
                    $(document).ready(function () {
                        ...
                        $('#lnkViewVideo').fancybox({
                            'padding': 0, 'showCloseButton': true, 'modal': true,'overlayShow': false, 'overlayOpacity': 0, 'centerOnScroll': false,
                            'titleShow': false, 'autoScale': false, 'width': 900,'height': 585, 'hideOnContentClick': false, 'transitionIn': 'none', 'transitionOut': 'none'
                        });
                    });
                </script>
            </div>
         ...
         <% if (VideoLink.ToString() != "" && VideoLink.ToString() != "#")
             { %>
        <div class="col s12 m12 l5 xl7" itemprop="description" > 
            <% }
    else
    { %>   <div class="col s12 m6 l6 xl7" itemprop="description" ><% } %>
            <%: ClientDesc %>
        </div>
        </div>
        <div class="row">
...
<div class="section" style="background-color:#CCCCCC;">
   ...
   <div class="card blue-grey darken-1">
        <div class="card-content white-text">
          <span class="card-title"><a href="<%=dr["URL"]%>" target="_blank"><%= dr["HEADING"]%></a></span><br>
        ...
        <div class="col s12 m6 l5">
            ...
            <div id="divAddInfo" class="related white-text" >
                <h5>This Company Can Be Found On</h5>
                    <ul class="twocols profile"> <% foreach (var dr in RelatedCategories) {  %>
...
<h5 style="font-size:40px;"><i class="fas fa-comments"></i> Company Reviews</h5>
    ...
    <script type='text/javascript'>
                    var recaptcha1;  var recaptcha2;  var recaptcha3;                
        var captchaCallBack = function () {            
            if ($("#hdnEmailCaptcha").val() != 'yes') {
                recaptcha1 = grecaptcha.render('recaptcha1', {
                    'sitekey': '6Lc72zMUAAAAABk1ajqMH-ThUswu6BIps5JS10s_', 'theme': 'light', 'callback':verify_callback1
                });
                $("#hdnEmailCaptcha").val('yes');
            }
            if ($("#recaptcha2").length > 0) {
                recaptcha2 = grecaptcha.render('recaptcha2', {
                    'sitekey': '6Lc72zMUAAAAABk1ajqMH-ThUswu6BIps5JS10s_', 'theme': 'light', 'callback': verify_callback2
                });
            }
            if ($("#recaptcha3").length > 0) {
                recaptcha3 = grecaptcha.render('recaptcha3', {
                    'sitekey': '6Lc72zMUAAAAABk1ajqMH-ThUswu6BIps5JS10s_', 'theme': 'light', 'callback': verify_callback3
                });
            }
        }
    </script>
    <script type='text/javascript'>
        $(document).ready(function () {
            $.get($('#hdnSrhRootPath').val() + 'statesearch.html', function (data) { $('#searchBarDir').html(data); });
        ...
        function verify_callback2(response) {
            $('#hiddenRecaptcha2').val(response); $("#frmShare").valid();
        }
        function verify_callback3(response) {
            $('#hiddenRecaptcha3').val(response); $("#frmRegCommenter").valid();
        }
    </script>
